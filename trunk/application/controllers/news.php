...
class News_Controller extends Template_Controller {
    ...
    public function view($article_id) {
        ...
        if (!$article->loaded || ($article->status != Article_Model::STATUS_PUBLISHED && !$this->auth_user->has_role('administrator')))
            ...
            $this->content->prev_article = ($this->auth_user && $this->auth_user->has_role('administrator')) ? ORM::factory('article')->where('id <' , $article_id)->where('status' , Article_Model::STATUS_PUBLISHED)->orderby('id' , 'DESC')->limit(1)->find()
            : ORM::factory('article')->where('id <' , $article_id)->orderby('id' , 'DESC')->limit(1)->find();
            $this->content->next_article = ($this->auth_user && !$this->auth_user->has_role('administrator')) ? ORM::factory('article')->where('id >' , $article_id)->where('status' , Article_Model::STATUS_PUBLISHED)->orderby('id' , 'ASC')->limit(1)->find()
